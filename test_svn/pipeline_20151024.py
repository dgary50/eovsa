...
# Alt version of rd_miriad_tsys, uses aipy, but inputs a list of files, rather than a time range
def rd_miriad_tsys_alt(filelist):
    ...
    for filename in filelist:
        ...
        delay.append(uv['delay'])
        for preamble, data in uv.all():
            # Look for time change
            if preamble[1] != ut:
                # Time has changed, so read new xtsys and ytsys, and delay
                ut = preamble[1]
                xtsys.append(uv['xtsys'])
                ytsys.append(uv['ytsys'])
                delay.append(uv['delay'])
    ...
    tsys = array((xtsys,ytsys))
    tsys = swapaxes(tsys,1,3)  # Order is now npol, nants, nf, nt
    tsys = swapaxes(tsys,0,1)  # Order is now nants, npol, nf, nt, as desired
    good, = where(tsys.sum(0).sum(0).sum(1) != 0.0)
    delay = array(delay)
    delay.shape = (len(utd), nants)
    tsys = tsys[:,:,good,:]
    fghz = fghz[good]
    return {'source':src, 'fghz':fghz, 'ut_mjd':utd, 'tsys':tsys, 'delay':delay}
    
#END of rd_miriad_tsys_alt
